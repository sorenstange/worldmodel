...
def load_data(cfg, mode = 'train'):
    ...
    df = pd.read_csv(f'./src/autotrader/LeWM/data/processed/{tf}/{symbol}.csv')
    df.set_index('OpenTime', inplace = True)
  
    df = df[((df.index >= cfg[f'{mode}_period'][0]) & (df.index < cfg[f'{mode}_period'][1]))]
...
def update_data(cfg):
    ...
    start_date  = min([min(cfg[period]) for period in ['train_period', 'val_period', 'test_period']])
    end_date    = max([max(cfg[period]) for period in ['train_period', 'val_period', 'test_period']])
    os.makedirs(f'./src/autotrader/LeWM/data/raw/{tf}', exist_ok=True)
...
def preprocess_data(cfg):
    ...
    tf      = cfg['timeframe']
    symbol = cfg['symbol']
    os.makedirs(f'./src/autotrader/LeWM/data/processed/{tf}', exist_ok=True)
    ...
    try:
        ...
        mu = df['Close'].rolling(window = cfg['normalization_window_size']).mean()
        std = df['Close'].rolling(window = cfg['normalization_window_size']).std()
        for col in ['Open', 'High', 'Low', 'Close']:
            df[col] = (df[col] - mu) / (std + 1e-8)

        df['Volume'] = np.log1p(df['Volume'])
        df['Volume'] = (df['Volume'] - df['Volume'].rolling(window = cfg['normalization_window_size']).mean()) / (df['Volume'].rolling(window = cfg['normalization_window_size']).std() + 1e-8)
        ...
        path = f'./src/autotrader/LeWM/data/processed/{tf}/{symbol}.csv'
        df.to_csv(path)
    except Exception as e:
        logger.error(f'Error at {symbol}: {e}')   
...
def main(cfg):
    from autotrader.util import set_logger
...
if __name__ == '__main__':
    ...
    cfg = OmegaConf.load('./src/autotrader/LeWM/config.yaml')
    main(cfg)
